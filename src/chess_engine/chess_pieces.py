...
class King(ChessPiece):
    ...
    def validate_castling(self, board, new_pos):
        if not self.moved:
            ...
            if (
                left
                and board[self.pos.x][0].string_representation() == "R"
                and not board[self.pos.x][0].moved
            ):
                new_pos.append(Pos(self.pos.x, 2, Move.CASTLING))
            if (
                right
                and board[self.pos.x][7].string_representation() == "R"
                and not board[self.pos.x][7].moved
            ):
...
class Pawn(ChessPiece):
    ...
    def get_new_possible_pos(self, board):
        ...
        if (
            self.validate_move_pawn([self.multiplier * 1, 0], board, new_pos)
            and not self.moved
        ):
    ...
    def validate_move_pawn(self, move, board, new_positions):
        ...
        if (
            self.validate_next_pos(new_pos, board)
    ...
    def en_passant(self, board, new_positions):
        ...
        if (
            ...
            new_positions.append(
                Pos(self.pos.x + self.multiplier * 1, self.pos.y - 1, Move.ENPASSANT)
            )
